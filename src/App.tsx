...
export default function App() {
  ...
  const handleFindPeace = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/serenity", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({ feeling, guidanceType })
      });
      ...
      const data: SerenityResponse = await response.json();
      setResult(data);
    } catch (err: any) {
      setError(err.message || "Failed to reach peace server. Please try again.");
    } finally {
      setLoading(false);
    }
  };
